// Section 1output_data - Define Global Variables
...
$('#strategy_new-run').click(function(event){
	...
	if (plot_code == "") {
		...
		// Define indicator_code object to pass into worker
		var indicator_code = [];
		// Get CodeMirror
		...
		for(i=0;i<indicator_name.length;i++) {
			var indicator_obj = document.getElementById(indicator_name[i]);
			if (indicator_obj !== null) {
				indicator_code.push(indicator_obj.value);
			}; 
		};
		worker.postMessage({
			"cmd": "run",
...
$("#strategy_new-import").click(function() {
	...
	worker.postMessage({"cmd": "import", "stockid": stockid});
});

// Click Save Indicator
$('#strategy_new-save').click(function(event){
